...
const SHELL = [
  ...
  './supabase-config.js', './manifest.webmanifest', './i18n-en.js',
...
self.addEventListener('fetch', (e) => {
  ...
  // ⚠⚠ คำขอปลอมจากเทมเพลต — ต้นเหตุที่หน้าเว็บค้างเป็นนาที (22 ก.ย. 2569)
  //   .dc.html เขียน src="{{ frSrc }}" ไว้ เบราว์เซอร์ยิงออกไปจริงก่อนคอมโพเนนต์จะแทนค่า
  //   หน้าเดียวมีแบบนี้ ~38 ใบ · 404 ทุกใบ แต่ยังกินคอนเนกชันและรอได้ถึง 8 วิต่อใบ
  //   ตอนแคชว่าง (เพิ่งขึ้นเวอร์ชันใหม่) มันไปแย่งคิวกับสคริปต์/รูปจริงจนโหลดไม่เสร็จ
  //   วัดจากเครื่องเจ้าของ: ใบที่ช้าสุด 107 วินาที · icon-192.png ของจริงก็โดนลากไป 106 วินาที
  //   → ตัดทิ้งตรงนี้เลย ไม่ต้องออกเน็ต ไม่ต้องแตะแคช
  //   (ทางแก้ที่ต้นเหตุคือไม่ให้เทมเพลตพ่น src ที่ยังไม่แทนค่า แต่มีหลายจุดทั่วไฟล์ — ยังไม่แตะ)
  if (req.url.indexOf('%7B%7B') !== -1 || req.url.indexOf('{{') !== -1) {
    e.respondWith(new Response('', { status: 404, statusText: 'template placeholder' }));
    return;
  }
